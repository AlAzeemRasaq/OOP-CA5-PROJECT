...
public class MenuSystem {
    ...
    // question 12
    public void deleteEntityById(Scanner scanner) throws DaoException {
        try {
            // Prompt the user to enter the entity ID to delete
            System.out.print("Enter ID to delete: ");
            int entityId = scanner.nextInt();

            PlayerDTO entity = playerDao.deleteEntityById(entityId);

            if (entity == null) {
                System.out.println("Player not found with ID: " + entityId);
                return; // Exit the method if entity not found
            }
            playerDao.deleteEntityById(entityId);
